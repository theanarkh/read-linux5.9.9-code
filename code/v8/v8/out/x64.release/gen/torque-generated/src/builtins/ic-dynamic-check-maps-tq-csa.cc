...
TNode<Int32T> DynamicCheckMaps_0(compiler::CodeAssemblerState* state_, TNode<Map> p_actualMap, TNode<IntPtrT> p_slotIndex, TNode<Object> p_actualHandler) {
  ...
  if (block0.is_used()) {
    ...
    tmp1 = CodeStubAssembler(state_).LoadFeedbackVectorSlot(TNode<FeedbackVector>{tmp0}, TNode<IntPtrT>{p_slotIndex});
    compiler::CodeAssemblerLabel label3(&ca_);
    tmp2 = CodeStubAssembler(state_).GetHeapObjectIfStrong(TNode<MaybeObject>{tmp1}, &label3);
    ca_.Goto(&block6);
    if (label3.is_used()) {
      ca_.Bind(&label3);
      ca_.Goto(&block7);
  ...
  if (block7.is_used()) {
    ca_.Bind(&block7);
    compiler::CodeAssemblerLabel label5(&ca_);
    tmp4 = CodeStubAssembler(state_).GetHeapObjectAssumeWeak(TNode<MaybeObject>{tmp1}, &label5);
    ca_.Goto(&block8);
    if (label5.is_used()) {
      ca_.Bind(&label5);
      ca_.Goto(&block9);
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp6 = PerformPolymorphicCheck_0(state_, TNode<HeapObject>{tmp2}, TNode<Map>{p_actualMap}, TNode<Object>{p_actualHandler});
  ...
  if (block9.is_used()) {
    ca_.Bind(&block9);
    tmp7 = FromConstexpr_int32_constexpr_int32_0(state_, static_cast<int>(DynamicCheckMapsStatus::kDeopt));
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp8 = PerformMonomorphicCheck_0(state_, TNode<FeedbackVector>{tmp0}, TNode<IntPtrT>{p_slotIndex}, TNode<HeapObject>{tmp4}, TNode<Map>{p_actualMap}, TNode<Object>{p_actualHandler});
