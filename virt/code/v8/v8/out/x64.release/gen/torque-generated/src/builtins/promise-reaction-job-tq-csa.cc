...
TNode<Object> RejectPromiseReactionJob_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<HeapObject> p_promiseOrCapability, TNode<Object> p_reason, PromiseReaction::Type p_reactionType) {
  ...
  if (block7.is_used()) {
    ca_.Bind(&block7);
    tmp4 = False_0(state_);
    tmp5 = ca_.CallStub<Object>(Builtins::CallableFor(ca_.isolate(), Builtins::kRejectPromise), p_context, tmp0, p_reason, tmp4);
  ...
  if (block3.is_used()) {
    ...
    tmp14 = ca_.CallStub<Object>(Builtins::CallableFor(ca_.isolate(), Builtins::kPromiseRejectReactionJob), p_context, p_reason, tmp13, p_promiseOrCapability);
...
TNode<Object> FuflfillPromiseReactionJob_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<HeapObject> p_promiseOrCapability, TNode<Object> p_result, PromiseReaction::Type p_reactionType) {
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp4 = ca_.CallStub<Object>(Builtins::CallableFor(ca_.isolate(), Builtins::kResolvePromise), p_context, tmp0, p_result);
