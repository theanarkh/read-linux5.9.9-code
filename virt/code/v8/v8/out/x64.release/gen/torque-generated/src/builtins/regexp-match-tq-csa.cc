...
TNode<Object> RegExpPrototypeMatchBody_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSReceiver> p_regexp, TNode<String> p_string, bool p_isFastPath) {
  ...
  if (block51.is_used()) {
    ...
    tmp54 = ca_.CallStub<String>(Builtins::CallableFor(ca_.isolate(), Builtins::kSubString), p_context, p_string, tmp52, tmp53);
...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp5 = ca_.CallStub<Object>(Builtins::CallableFor(ca_.isolate(), Builtins::kRegExpMatchFast), parameter0, tmp2, tmp1);
