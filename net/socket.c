...
// 创建socket层使用的vfs inode（socket_alloc）
 ...
 */
// 分配一个socket_alloc结构体，然后返回里面的socket地址
struct socket *sock_alloc(void)
{
	struct inode *inode;
	struct socket *sock;
	// 分配一个inode，由socket.c提供的钩子函数
	inode = new_inode_pseudo(sock_mnt->mnt_sb);
	if (!inode)
		return NULL;
	// 根据inode找到socket_alloc地址，再找到socket地址
	sock = SOCKET_I(inode);
