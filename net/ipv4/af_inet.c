...
lookup_protocol:
	err = -ESOCKTNOSUPPORT;
	rcu_read_lock();
	// 根据数据类型找到处理函数集，比如SOCK_STREAM、SOCK_DGRAM，保存到answer中
	...
	err = -ENOBUFS;
	// 分配一个sock结构体，表示inet层，并设置函数集为answer_prot
	...
	// 转成inet_sock，inet_sock是sock结构体的“子类”，分配sk_alloc的时候分配了足够的内存了
	inet = inet_sk(sk);
...
#endif /* CONFIG_COMPAT */
// 提供给用户侧的接口在这里定义，这些函数可能会调用下一层协议的接口，也可能在inet就实现了全部逻辑
...
static int __init inet_init(void)
{
	...
	for (r = &inetsw[0]; r < &inetsw[SOCK_MAX]; ++r)
		INIT_LIST_HEAD(r);
	// 注册使用网络层时，数据类型（socket函数的type字段）和处理函数
